...
class PatientPortal extends BasePage {
    ...
    private usernameLogin: string= 'automation9@email.com';
    ...
    proceedLogin() : void {
        cy.wait(5000).get('.inp').eq(0).click().type('automation9@email.com');
    ...
    bookNewAppointmentASAPonUniversities(): void {
        ...
        this.checkAvailability();
        //this.searchAM();
        cy.get(this.radioTabSelector).eq(25).click({force:true})
        cy.wait(4400)
        cy.contains('Confirm Appointment').click();
        cy.wait(12500);
        cy.contains('Your appointment was successfully booked').should('be.visible');
    ...
    forgotPassword() : void{
        ...
        cy.get(this.forgotFieldSelector).click().type('automation9@email.com')
